...
docker build --build-arg "APP_VERSION=`git describe --tags`" -t="stepniewski.tech/logger:`git describe --tags`" .
...
echo -e "   \e[92m\e[1m scp docker/target/logger-`git describe --tags`.tar.gz wheely@earth.stepniewski.tech:~ \\e[033;0m"